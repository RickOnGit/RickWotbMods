function installer() {
  ...
  "🤖 Android")
    ...
    # rm -rf "$tmpBackupDir"
    echo -e "📥 ${BOL}${GREEN}$model${NC} installed for ${BOL}${ORANGE}$mainName${NC}" >>$tmpLogs
    ;;
  *)
    gum spin -s "pulse" --spinner.foreground="208" --title "🗂️ Installing..." --title.foreground="245" -- rsync -a --backup --backup-dir="$tmpBackupDir" "$sourceDir/Data/" "$wotbData"
    gum spin -s "pulse" --spinner.foreground="208" --title "💾 Doing a file backup..." --title.foreground="245" -- rsync -au "$tmpBackupDir/" "$wotbBackup"
    rm -rf "$tmpBackupDir"
    echo -e "📥 ${ITAL}${GREEN}$model${NC} installed for ${BOL}${ORANGE}$mainName${NC}" >>$tmpLogs
    ;;
  esac
